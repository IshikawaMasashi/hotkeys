...
let _downKeys: number[] = []; // 记录摁下的绑定键

let _scope = 'all'; // 默认热键范围
const elementHasBindEvent: (HTMLElement | Document)[] = []; // 已绑定事件的节点记录
...
export function filter(event: KeyboardEvent) {
  const target = event.target as HTMLElement; //event.target || event.srcElement;
...
export function isPressed(keyCode: any) {
...
export function deleteScope(scope: any, newScope?: any) {
  let handlers;
  let i;
...
export function unbind(keysInfo?: any, ...args: any[]) {
...
// 解除绑定某个范围的快捷键
const eachUnbind = ({ key, scope, method, splitKey = '+' }: any) => {
  const multipleKeys = getKeys(key);
  multipleKeys.forEach((originKey: any) => {
...
function dispatch(event: any) {
  ...
  if (key in _mods) {
    ...
    for (const k in _modifier) {
      if (_modifier[k] === key) (hotkeys as any)[k] = true;
    }
...
// 判断 element 是否已经绑定事件
function isElementBind(element: HTMLElement | Document) {
  return elementHasBindEvent.indexOf(element) > -1;
}

function hotkeys(key: any, option?: any, method?: any) {
  _downKeys = [];
